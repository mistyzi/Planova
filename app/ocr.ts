...
async function prepareLocalImage(
  ...
  const destination =
    `${FileSystem.cacheDirectory}ocr-${Date.now()}${extension}`;

  console.log(
    "[OCR] Copying image to cache:",
    destination
  );
  ...
  if (!copiedInfo.exists) {
    throw new Error(
      "Could not create a local copy of the image."
    );
  }
...
function getOCRApiError(
  ...
  if (
    typeof data.OCRExitCode === "number" &&
    data.OCRExitCode !== 1
  ) {
    if (
      Array.isArray(data.ErrorMessage)
...
export async function scanImage(
  ...
  console.log(
    `[OCR] SCANNING IMAGE - ATTEMPT ${attempt}/3`
  );
  ...
  if (!OCR_API_KEY) {
    throw new Error(
      "OCR API key is missing. Check utils/ocr.ts."
    );
  }
  ...
  try {
    ...
    const localUri =
      await prepareLocalImage(imageUri);
    ...
    console.log(
      "[OCR] Sending request to OCR.space..."
    );
    ...
    if (!response.ok) {
      ...
      if (
        ...
        console.log(
          `[OCR] RETRYING AFTER HTTP ${response.status}`
        );
    ...
    if (attempt < 3) {
      console.log(
        `[OCR] RETRYING IMAGE (${attempt + 1}/3)`
      );
...
export async function scanMultipleImages(
  ...
  for (
    ...
    console.log(
      `[OCR] PROCESSING PAGE ${pageNumber}/${validUris.length}`
    );
