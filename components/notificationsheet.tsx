...
const NOTIFICATIONS_STORAGE_KEY =
  "@planova_notifications";

const DISMISSED_NOTIFICATIONS_STORAGE_KEY =
  "@planova_dismissed_notifications";

const TASK_SNAPSHOT_STORAGE_KEY =
  "@planova_task_snapshot";

const LAST_ACTIVE_STORAGE_KEY =
  "@planova_last_active";

const RETURN_REMINDER_STORAGE_KEY =
  "@planova_return_reminder";
...
export default function NotificationPopup() {
  ...
  const addNotification = async (
    notification: PlanovaNotification
  ) => {
    try {
      const dismissed =
        await loadDismissedNotificationIds();
      ...
      const existing: PlanovaNotification[] =
  ...
  const checkCompletedTasks = async (
    ...
    try {
      ...
      for (const task of tasks) {
        currentSnapshot[task.id] = {
          completed: task.completed,
        };

        const previous =
          previousSnapshot[task.id];
      ...
      console.log(
        "Failed to check completed tasks:",
        error
      );
    }
  };
    ...
    async () => {
      try {
        ...
        await addNotification({
          ...
          message:
            "You haven't checked in today. Take a moment to log in and keep your streak alive!",
        ...
        console.log(
          "Failed to check streak reminder:",
          error
        );
      }
    };
  ...
  const checkReturnReminder =
    async () => {
      try {
          ...
          if (
            ...
            if (
              ...
              await addNotification({
                ...
                title:
                  "Your Starpath Misses You ✦",

                message:
                  "It's been a while. Come back and study with us — your next step is waiting.",
        ...
        console.log(
          "Failed to check return reminder:",
          error
        );
      }
    };
  ...
  const clearNotifications =
    async () => {
      try {
        ...
        console.log(
          "Failed to permanently clear notifications:",
          error
        );
      }
    };
